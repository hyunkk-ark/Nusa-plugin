...
class MigratorUtil {
    protected static boolean createDirBackup(File dir) {
        try {
            ...
            if (Files.exists(backupPath, LinkOption.NOFOLLOW_LINKS)) {
                Slimefun.logger().log(Level.WARNING, "Existing backup data detected, backup skipped");
                return true;
            }
                    ...
                    fs.forEach(path -> {
                        ...
                        try {
                            ...
                            Slimefun.logger()
                                    .log(
                                            Level.WARNING,
                                            "Back up old data " + dir.getName() + " Problem occurs when",
                                            e);
                        }
                    });
                }
            ...
            Slimefun.logger().log(Level.WARNING, "Back up old data " + dir.getName() + " Problem occurs when", e);
            return false;
        }
    }
    ...
    protected static void deleteOldFolder(File dir) {
        try {
            ...
            Slimefun.logger().log(Level.WARNING, "delete folder " + dir.getAbsolutePath() + " Problem occurs when", e);
        }
    }
