    ...
    public MigrateStatus migrateData() {
        ...
        for (File file : listFiles) {
            ...
            try {
                ...
                if (!p.hasPlayedBefore()) {
                    Slimefun.logger()
                            .log(Level.INFO, "Skipping data for a player who has never joined this server: " + uuid);
                    total--;
                    continue;
                }
                ...
                Slimefun.logger()
                        .log(Level.WARNING, "Illegal named player data file detected: '" + file.getName() + "'");
        ...
        if (MigratorUtil.createDirBackup(playerFolder)) {
            Slimefun.logger()
                    .log(
                            Level.INFO,
                            "Successfully migrated {0} player profiles. A backup is stored in ./data-storage/Slimefun/old_data.",
                            migratedCount);
            try {
