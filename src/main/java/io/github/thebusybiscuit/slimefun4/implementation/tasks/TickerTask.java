...
public class TickerTask implements Runnable {
    ...
    private void reportErrors(Location l, SlimefunItem item, Throwable x) {
        ...
        if (errors == 1) {
            ...
            Slimefun.logger()
                    .log(
                            Level.SEVERE,
                            "Multiple errors occurred in the past 4 ticks and the machine corresponding to this block has been disabled.");
            Slimefun.logger().log(Level.SEVERE, "See /plugins/Slimefun/error-reports/ for details.");
            Slimefun.logger()
                    .log(Level.SEVERE, "When asking for help, send the error-report file instead of a screenshot.");
    ...
    /**
     * Returns the read-only collection of ticking locations for blocks with the
     * {@link UniversalBlock} attribute in the given chunk.
     *
     * <p>The returned locations may belong to loaded or unloaded chunks.
     *
     * @param chunk the {@link Chunk} to inspect
     * @return all machine ticking locations in the chunk
     */
    @Nonnull
