...
## English Paper Multi-Version (15 Jul 2026)

#### Changes
- Changed `api-version` in `plugin.yml` from `'26.1.2'` to `'1.21'` for broad cross-version compatibility (1.21 – 26.x).
- Updated Paper API dependency from pinned `26.1.2.build.74-stable` to dynamic `26.1.2.build.+`.
- Fixed `RECOMMENDED_JAVA_VERSION` constant in `VersionsCommand.java` to match actual Java 25 requirement.
- Updated README with multi-version support table and revised installation instructions.

## English Paper 26.1.2 (14 Jul 2026)

#### Changes
- Updated the compile target to the stable Paper `26.1.2.build.74-stable` API.
- Updated the Java toolchain and bytecode target from Java 21 to Java 25.
- Updated `plugin.yml` to require Paper API version `26.1.2`.
- Updated runtime notices, build instructions, JitPack configuration, and issue templates.
- Added a verified shaded `Slimefun-ENGLISH-26.1.2.jar` release artifact.
